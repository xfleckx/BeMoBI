...
//
// This DLL wrapper handles network communication with a PhaseSpace Impulse server.
//

namespace PhaseSpace
{
	public class libowlsock : MonoBehaviour
	{
		...
		// structs from owl_scan.h
		[StructLayout(LayoutKind.Sequential, Pack=1, CharSet=CharSet.Ansi)]
		public struct OWLConnection
		{			
		...
		private const string pluginname = "libowlsock";
#else
		private const string pluginname = "owlsock";
#endif
		...
		[DllImport(pluginname)]
		public static extern int owl_scan (string address, int port, string message, int timeout, [param: In,Out][MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 5)] OWLConnection[] connections, int count);
	}
}
